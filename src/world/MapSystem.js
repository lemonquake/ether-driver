...
// Geometries & Materials Caches to prevent GPU reallocation/compilation stutters
...
export function getBoxGeometry(w, h, d) {
  ...
  return getCachedGeometry(key, () => new THREE.BoxGeometry(w, h, d));
}

export function getCylinderGeometry(rt, rb, h, s) {
  const key = `cyl_${rt}_${rb}_${h}_${s}`;
  return getCachedGeometry(key, () => new THREE.CylinderGeometry(rt, rb, h, s));
}

export function getSphereGeometry(r, wS, hS) {
  const key = `sphere_${r}_${wS}_${hS}`;
  return getCachedGeometry(key, () => new THREE.SphereGeometry(r, wS, hS));
}

export function getTorusGeometry(r, t, rS, tS) {
  const key = `torus_${r}_${t}_${rS}_${tS}`;
  return getCachedGeometry(key, () => new THREE.TorusGeometry(r, t, rS, tS));
}

export function getConeGeometry(r, h, s) {
  const key = `cone_${r}_${h}_${s}`;
  return getCachedGeometry(key, () => new THREE.ConeGeometry(r, h, s));
}
...
export function addRamp(ctx, materials, x, z, w, d, yaw, hStart, hEnd) {
  ...
  const geomKey = `wedge_${w}_${d}_${hStart}_${hEnd}`;
  const geom = getCachedGeometry(geomKey, () => createWedgeGeometry(w, d, hStart, hEnd));
  const material = getCachedMaterial('ramp_material', () => new THREE.MeshStandardMaterial({
    color: 0x3e4247,
    roughness: 0.88,
    metalness: 0.15,
...
export function addRoad(ctx, materials, x, z, w, d, y = 0.02) {
  const geom = getBoxGeometry(w, 0.08, d);
  const mesh = new THREE.Mesh(geom, materials.asphalt);
...
export function addBuilding(ctx, materials, x, z, w, h, d, material, collisionHeight = 0) {
  ...
  const body = new THREE.Mesh(getBoxGeometry(w, h, d), material);
  body.position.y = h / 2;
  body.castShadow = true;
  body.receiveShadow = true;
  group.add(body);
  const front = new THREE.Mesh(getBoxGeometry(w * 0.82, h * 0.82, 0.04), materials.windows);
  front.position.set(0, h * 0.52, -d / 2 - 0.024);
  group.add(front);
  const roof = new THREE.Mesh(getBoxGeometry(w * 1.04, 0.22, d * 1.04), materials.concrete);
...
export function addBarrier(ctx, materials, x, z, r = 0) {
  ...
  const group = new THREE.Group();
  const geom = getBoxGeometry(5.4, 0.55, 0.42);
  const base = new THREE.Mesh(geom, materials.hazard);
...
export function addCrateStack(ctx, materials, x, z) {
  ...
  for (let i = 0; i < 3; i += 1) {
    // Round size to 2 decimal places to limit unique geometries in the cache
    const size = Math.round((1.2 + Math.random() * 0.5) * 100) / 100;
    const geom = getBoxGeometry(size, size, size);
    const mesh = new THREE.Mesh(geom, materials.brick);
...
export function addParkedCar(ctx, materials, x, z, color, r = 0) {
  ...
  const group = new THREE.Group();
  const paint = getCachedMaterial(`car_paint_${color}`, () => new THREE.MeshStandardMaterial({ color, roughness: 0.35, metalness: 0.45 }));
  const base = new THREE.Mesh(getBoxGeometry(2.0, 0.55, 3.7), paint);
  base.position.y = 0.62;
  group.add(base);
  const cabin = new THREE.Mesh(getBoxGeometry(1.45, 0.7, 1.45), materials.glass);
...
export function addLamp(ctx, materials, x, z) {
  ...
  const pole = new THREE.Mesh(getCylinderGeometry(0.07, 0.11, 5.4, 12), materials.lamp);
  pole.position.y = 2.7;
  ...
  const arm = new THREE.Mesh(getBoxGeometry(1.6, 0.08, 0.08), materials.lamp);
  arm.position.set(0.8, 5.3, 0);
  arm.castShadow = true;
  group.add(arm);

  // 3. Futuristic lamp head hood cover
  const hood = new THREE.Mesh(getBoxGeometry(0.5, 0.16, 0.35), materials.concrete);
  hood.position.set(1.6, 5.2, 0);
  hood.castShadow = true;
  group.add(hood);

  // 4. Emissive glowing bulb
  const bulbMat = getCachedMaterial('lamp_bulb_mat', () => new THREE.MeshBasicMaterial({ color: 0xffeab3 }));
  const bulb = new THREE.Mesh(getBoxGeometry(0.35, 0.05, 0.25), bulbMat);
...
export function addBorderWalls(ctx, size, wallColor, materials) {
  ...
  const neonMat = getCachedMaterial(`neon_wall_${wallColor}`, () => new THREE.MeshStandardMaterial({
    color: wallColor,
    emissive: wallColor,
  ...
  const buildOneSideWall = (length, x, z, angle) => {
    ...
    // Concrete base wall
    const base = new THREE.Mesh(getBoxGeometry(length, 2.8, wallThickness), borderMat);
    base.position.y = 1.4;
    base.castShadow = true;
    base.receiveShadow = true;
    sideGroup.add(base);
    ...
    const pillarGeom = getCylinderGeometry(0.45, 0.65, 7.5, 8);
    const capGeom = getSphereGeometry(0.55, 8, 8);
    const bandGeom = getTorusGeometry(0.55, 0.08, 4, 8);
    const panelGeom = getBoxGeometry(pillarSpacing - 2.5, 1.25, 0.12);
    ...
    for (let pos = -halfLen; pos <= halfLen; pos += pillarSpacing) {
      ...
      // Glowing power cap
      const cap = new THREE.Mesh(capGeom, neonMat);
      cap.position.set(pos, 7.5, 0);
      sideGroup.add(cap);

      // Accent warning bands on the pillars
      const band = new THREE.Mesh(bandGeom, neonMat);
    ...
    const railUpper = new THREE.Mesh(getBoxGeometry(length, 0.22, 0.22), neonMat);
    railUpper.position.set(0, 5.0, wallThickness * 0.45 + 0.12);
    sideGroup.add(railUpper);

    const railLower = new THREE.Mesh(getBoxGeometry(length, 0.22, 0.22), neonMat);
    ...
    for (let pos = -halfLen + pillarSpacing * 0.5; pos < halfLen; pos += pillarSpacing) {
      const panel = new THREE.Mesh(panelGeom, materials.hazard);
  ...
  if (ctx.mapGroup) {
    ctx.mapGroup.add(bordersGroup);
  } else {
    ctx.scene.add(bordersGroup);
  }
...
export function addTrafficArch(ctx, x, z, angle, materials) {
  ...
  const poleL = new THREE.Mesh(getCylinderGeometry(0.12, 0.16, 7.8, 8), metal);
  poleL.position.set(-11.5, 3.9, 0);
  poleL.castShadow = true;
  group.add(poleL);

  // Right supporting pillar
  const poleR = new THREE.Mesh(getCylinderGeometry(0.12, 0.16, 7.8, 8), metal);
  poleR.position.set(11.5, 3.9, 0);
  poleR.castShadow = true;
  group.add(poleR);

  // Overhead horizontal beam
  const beam = new THREE.Mesh(getBoxGeometry(23, 0.22, 0.22), metal);
  beam.position.set(0, 7.6, 0);
  beam.castShadow = true;
  group.add(beam);

  // Tech details: glowing energy core in center of beam
  const coreMat = getCachedMaterial('arch_core_mat', () => new THREE.MeshBasicMaterial({ color: 0x82ffcf }));
  const core = new THREE.Mesh(getBoxGeometry(3, 0.1, 0.3), coreMat);
  core.position.set(0, 7.6, 0);
  group.add(core);

  // Traffic lights hanging above each lane
  const boxMat = getCachedMaterial('arch_box_mat', () => new THREE.MeshStandardMaterial({ color: 0x14181c, roughness: 0.35 }));
  const redGlow = getCachedMaterial('arch_red_glow', () => new THREE.MeshBasicMaterial({ color: 0xff1f4f }));
  const greenGlow = getCachedMaterial('arch_green_glow', () => new THREE.MeshBasicMaterial({ color: 0x00ff66 }));

  const lightGeom = getBoxGeometry(0.5, 1.1, 0.4);
  const lensGeom = getSphereGeometry(0.18, 8, 8);

  // 3 Lanes of traffic lights
  const offsets = [-7, 0, 7];
  offsets.forEach((offsetX, idx) => {
    const box = new THREE.Mesh(lightGeom, boxMat);
    ...
    const lens = new THREE.Mesh(lensGeom, isGreen ? greenGlow : redGlow);
...
export function addPipeline(ctx, x1, z1, x2, z2, height, materials) {
  ...
  const metal = materials.lamp;
  const glowMat = getCachedMaterial('pipeline_glow', () => new THREE.MeshStandardMaterial({
    color: 0xff4f00,
    emissive: 0xff4f00,
  ...
  // Key cylinder by exact distance
  const pipeGeom = getCylinderGeometry(0.68, 0.68, dist, 16);
  const pipe = new THREE.Mesh(pipeGeom, metal);
  ...
  const ringGeom = getTorusGeometry(0.78, 0.09, 6, 16);
  ...
  // Support geom varies by height, key it by height
  const supportGeom = getBoxGeometry(0.6, height + 0.2, 1.2);
  const collarGeom = getBoxGeometry(1.5, 0.3, 1.4);
  for (let offset = -halfLen + 2; offset <= halfLen - 2; offset += spacing) {
    const support = new THREE.Mesh(supportGeom, materials.concrete);
    ...
    const collar = new THREE.Mesh(collarGeom, metal);
...
export function addTeslaCoil(ctx, x, z, height, materials) {
  ...
  const metal = materials.lamp;
  const cyanGlow = getCachedMaterial('tesla_glow', () => new THREE.MeshStandardMaterial({
    color: 0x00ffcc,
    emissive: 0x00ffcc,
  ...
  const base = new THREE.Mesh(getCylinderGeometry(2.8, 3.3, 0.9, 8), materials.concrete);
  ...
  const hazardRing = new THREE.Mesh(getCylinderGeometry(2.9, 2.9, 0.3, 8), materials.hazard);
  hazardRing.position.y = 0.45;
  hazardRing.castShadow = true;
  group.add(hazardRing);

  // 3. Tapered metallic spire tower
  const spire = new THREE.Mesh(getCylinderGeometry(0.35, 1.1, height, 8), metal);
  ...
  for (let i = 0; i < ringCount; i += 1) {
    ...
    const coil = new THREE.Mesh(getTorusGeometry(radius, 0.14, 6, 16), cyanGlow);
  ...
  const sphere = new THREE.Mesh(getSphereGeometry(1.5, 16, 16), cyanGlow);
  sphere.position.y = height + 1.6;
  group.add(sphere);

  // 6. Horizontal orbiting capacitor rings on top
  const topRing = new THREE.Mesh(getTorusGeometry(2.3, 0.08, 4, 16), cyanGlow);
...
export function addTurboTile(ctx, materials, x, z, yaw, y = 0.02) {
  ...
  const base = new THREE.Mesh(getBoxGeometry(6, 0.1, 8), materials.concrete);
  base.position.y = 0.05;
  group.add(base);

  const glowMat = getCachedMaterial('turbo_glow', () => new THREE.MeshStandardMaterial({ color: 0xffaa00, emissive: 0xffaa00, emissiveIntensity: 2.5, transparent: true, opacity: 0.8 }));
  
  const coneGeom = getConeGeometry(2, 2, 3);
  const arrow1 = new THREE.Mesh(coneGeom, glowMat);
  arrow1.rotation.x = -Math.PI / 2;
  arrow1.position.set(0, 0.15, 1);
  const arrow2 = new THREE.Mesh(coneGeom, glowMat);
...
export function addJumpTile(ctx, materials, x, z, yaw, y = 0.02) {
  ...
  const base = new THREE.Mesh(getBoxGeometry(6, 0.1, 6), materials.concrete);
  base.position.y = 0.05;
  group.add(base);

  const glowMat = getCachedMaterial('jump_glow', () => new THREE.MeshStandardMaterial({ color: 0x00ffcc, emissive: 0x00ffcc, emissiveIntensity: 2.0, transparent: true, opacity: 0.8 }));
  const circle = new THREE.Mesh(getTorusGeometry(1.8, 0.3, 16, 32), glowMat);
  circle.rotation.x = -Math.PI / 2;
  circle.position.y = 0.15;
  
  const core = new THREE.Mesh(getCylinderGeometry(1, 1, 0.2, 16), glowMat);
